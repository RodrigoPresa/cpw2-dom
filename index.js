...
function background() {
    ...
    bgColor.style.backgroundColor = "#BECCC3";
...
function search() {
    ...
    if (inputName) {
        let filterNames = data.filter(function (el) { return el.name.toUpperCase().startsWith(inputName.toUpperCase()) });

        let nodeElement;
        filterNames.forEach(function (el, index) {
            nodeElement = document.createElement("div");
            nodeElement.innerHTML = el.name;
            result.appendChild(nodeElement);            
        });
    }    
}
